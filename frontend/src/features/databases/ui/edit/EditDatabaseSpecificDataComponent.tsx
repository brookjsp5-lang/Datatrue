...
import { EditMySqlPhysicalSpecificDataComponent } from './EditMySqlPhysicalSpecificDataComponent';
...
export const EditDatabaseSpecificDataComponent = ({
  ...
  const saveDb = async (databaseToSave: Database) => {
    ...
    if (databaseToSave.type === DatabaseType.MYSQL_PHYSICAL) {
      onSaved(databaseToSave);
      return;
    }
  ...
  switch (editingDatabase.type) {
    ...
    case DatabaseType.MYSQL_PHYSICAL:
      return <EditMySqlPhysicalSpecificDataComponent {...commonProps} />;
