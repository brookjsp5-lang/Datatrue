...
export const CreateDatabaseComponent = ({
  ...
  const isPhysical =
    database.type === DatabaseType.POSTGRES_PHYSICAL || database.type === DatabaseType.MYSQL_PHYSICAL;
  const isPostgres = isPostgresType(database.type);
  const isMysqlPhysical = database.type === DatabaseType.MYSQL_PHYSICAL;
    ...
    return (
      <EditDatabaseSpecificDataComponent
        ...
        onSaved={(database) => {
          ...
          setStep(isMysqlPhysical ? 'backup-config' : 'create-readonly-user');
        }}
      />
    );
  }
