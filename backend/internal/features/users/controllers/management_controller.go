...
func (c *ManagementController) RegisterRoutes(router *gin.RouterGroup) {
	...
	router.POST("/users", user_middleware.RequireRole(user_enums.UserRoleAdmin), c.CreateUser)
...
// CreateUser
// @Summary Create user
// @Description Create a user with an initial password (admin only)
// @Tags user-management
// @Accept json
// @Produce json
// @Security BearerAuth
// @Param request body users_dto.CreateUserRequestDTO true "User creation data"
// @Success 201 {object} users_dto.UserProfileResponseDTO
...
func (c *ManagementController) CreateUser(ctx *gin.Context) {
	...
	user, err := c.managementService.CreateUser(&request, currentUser)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	ctx.JSON(http.StatusCreated, user_dto.UserProfileResponseDTO{
		ID:        user.ID,
		Email:     user.Email,
		Name:      user.Name,
		Role:      user.Role,
