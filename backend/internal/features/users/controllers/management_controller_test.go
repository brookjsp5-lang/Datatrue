...
func Test_CreateUser_WhenUserIsAdmin_UserCreatedAndCanSignIn(t *testing.T) {
	router := createE2ETestRouter()
	adminUser := users_testing.CreateTestUser(users_enums.UserRoleAdmin)
	email := "created-" + uuid.New().String() + "@example.com"
	password := "created-password-123"

	request := users_dto.CreateUserRequestDTO{
		Email:    email,
		Name:     "Created User",
		Password: password,
	...
	assert.Equal(t, email, signInResponse.Email)
	assert.NotEmpty(t, signInResponse.Token)
}

func Test_CreateUser_WhenUserIsMember_ReturnsForbidden(t *testing.T) {
	router := createManagementTestRouter()
...
func Test_CreateUser_WithDuplicateEmail_ReturnsBadRequest(t *testing.T) {
	router := createManagementTestRouter()
	adminUser := users_testing.CreateTestUser(users_enums.UserRoleAdmin)
	existingUser := users_testing.CreateTestUser(users_enums.UserRoleMember)
...
func Test_CreateUser_WhenRegularAdminCreatesAdmin_ReturnsBadRequest(t *testing.T) {
	router := createManagementTestRouter()
	regularAdmin := users_testing.CreateTestUser(users_enums.UserRoleAdmin)

	request := users_dto.CreateUserRequestDTO{
		Email:    "admin-created-" + uuid.New().String() + "@example.com",
		Name:     "Created Admin",
		Password: "created-password-123",
	...
	assert.Contains(t, string(resp.Body), "only the root admin user can create admin accounts")
}

func Test_CreateUser_WhenRootAdminCreatesAdmin_UserCreated(t *testing.T) {
	router := createManagementTestRouter()
	...
	assert.Equal(t, users_enums.UserRoleAdmin, response.Role)
	assert.True(t, response.IsActive)
}
